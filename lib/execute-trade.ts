/**
 * lib/execute-trade.ts
 *
 * Shared trade execution logic extracted from app/api/trading/execute/route.ts.
 * Both the execute API route and the autoscan route call this directly,
 * eliminating the need for autoscan to make an HTTP round-trip (which failed
 * because the auth token was not forwarded — QA-007).
 */
...
const MOCK_PRICES: Record<string, number> = {
  "BTC/USDT": 67245.5, "ETH/USDT": 3521.8, "SOL/USDT": 142.3,
  "BNB/USDT": 598.4, "XRP/USDT": 0.625, "ADA/USDT": 0.452,
  "DOGE/USDT": 0.138, "AVAX/USDT": 38.75, "DOT/USDT": 7.82,
  "MATIC/USDT": 0.891, "EUR/USD": 1.0842, "GBP/USD": 1.2735,
  "USD/JPY": 149.85, "AUD/USD": 0.6521, "USD/CAD": 1.3612,
  "NZD/USD": 0.6043, "EUR/GBP": 0.8512, "EUR/JPY": 162.48,
  "GBP/JPY": 190.75, "USD/CHF": 0.8923, "AAPL": 189.45,
  "MSFT": 378.2, "TSLA": 245.6, "NVDA": 875.3,
  "GOOGL": 168.9, "AMZN": 185.7, "META": 495.3,
  "SPY": 521.4, "QQQ": 447.8,
};

const SECRETS_PATH = join(TMP_DIR, ".broker-secrets.json");
const CONNECTIONS_PATH = join(TMP_DIR, "broker-connections.json");
...
// ---------------------------------------------------------------------------
// Helper: live price with 30s cache
// ---------------------------------------------------------------------------
...
function regimeLabel(regime: string): string {
  const labels: Record<string, string> = {
    strong_bull: "STRONG BULL", bull: "BULL", moderate_bull: "MODERATE BULL",
    weak_bull: "WEAK BULL", sideways: "SIDEWAYS", choppy: "CHOPPY",
    neutral: "NEUTRAL", weak_bear: "WEAK BEAR", bear: "BEAR", crash: "CRASH",
  };
...
async function executeLiveTrade(
  ...
  if (!exchangeId) {
    throw new Error(`Live trading is not supported for "${brokerId}". Supported: ${Object.keys(EXCHANGE_MAP).join(", ")}`);
  }
    ...
    if (orderType === "market") {
      if (side === "buy" && exchangeId === "coinbase") {
        ...
        if (price && price > 0) {
          ...
          order = await exchange.createOrder(normalizedSymbol, "market", side, quantity, price, orderParams);
        } else {
          const fallbackCost = quantity * 87000;
        ...
        order = await exchange.createOrder(normalizedSymbol, "market", side, quantity, undefined, orderParams);
      }
    } else {
      if (!limitPrice) {
      ...
      order = await exchange.createOrder(normalizedSymbol, "limit", side, quantity, limitPrice, orderParams);
    }
  } catch (err: unknown) {
...
export async function executeTrade(params: ExecuteTradeParams): Promise<ExecuteTradeResult> {
  ...
  if (!symbol || !side || !quantity) {
    return { success: false, status: 400, data: { success: false, message: "symbol, side, and quantity are required" } };
  }
  if (!["LONG", "SHORT"].includes(side)) {
    return { success: false, status: 400, data: { success: false, message: "side must be LONG or SHORT" } };
  }
  if (quantity <= 0) {
    return { success: false, status: 400, data: { success: false, message: "quantity must be positive" } };
  }
